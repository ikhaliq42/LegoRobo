...
int SurfDemo(Robot *robo) {
	...
	while(char(waitKey(1)) != 'q') {
		...
		for(int i = 0; i < matches.size(); i++) {
			if(matches[i][0].distance < 0.6 * matches[i][1].distance) good_matches.push_back(matches[i][0]);
		}

		
		//Indicate with movement if there is a match
		if (good_matches.size() >= matchThreshold) {
			robo->Turn(0,3,defaultPower);
			robo->Turn(1,3,defaultPower);
			robo->Turn(0,3,defaultPower);
			robo->Turn(1,3,defaultPower);
		}
